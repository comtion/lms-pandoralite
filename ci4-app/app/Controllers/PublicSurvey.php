...
class PublicSurvey extends BaseController
{
    ...
    public function report(int $id){$u=$this->session->get('user');if(!(new PermissionModel())->can($u,'survey/report_survey'))return redirect()->to(site_url('dashboard'));$m=new PublicSurveyModel();return view('public_survey/report',['survey'=>$m->survey($id,$this->session->get('lang')??'english'),'rows'=>$m->report($id,$u)]);}
    public function reportIndex(){ $u=$this->session->get('user');if(!(new PermissionModel())->can($u,'survey/report_survey'))return redirect()->to(site_url('dashboard'));return view('public_survey/report_index',['surveys'=>(new PublicSurveyModel())->allSurveys($this->session->get('lang')??'english')]); }
