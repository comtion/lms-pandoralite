...
class Manage extends CI_Controller {
	...
	public function fetch_detail_userenroll(){
		...
		$this->load->model('Manage_model', 'manage', FALSE);
		$this->manage->loadDB();
		$query = $this->manage->fetch_data_userenroll($_REQUEST['emp_id']);
		$num = 1;
      	$draw = intval($this->input->get("draw"));
      	$start = intval($this->input->get("start"));
      	$length = intval($this->input->get("length"));
	...
	public function fetch_detail_learnerincomplete(){
		...
		$this->load->model('Manage_model', 'manage', FALSE);
		$this->manage->loadDB();
		$query = $this->manage->fetch_data_learnerincomplete();
		$num = 1;
      	$draw = intval($this->input->get("draw"));
      	$start = intval($this->input->get("start"));
      	$length = intval($this->input->get("length"));
	...
	public function fetch_detail_department(){
		...
		$this->load->model('Manage_model', 'manage', FALSE);
		$this->manage->loadDB();
		$query = $this->manage->fetch_data_department();
		$num = 1;
      	$draw = intval($this->input->get("draw"));
      	$start = intval($this->input->get("start"));
      	$length = intval($this->input->get("length"));
		...
		if(countArray($_REQUEST)>0){
			...
			if($chkmatch==1){
					...
					if($emp_manage_a!=""){
							...
							if(isset($fetch_manage['is_manager']) && $fetch_manage['is_manager']=="0"){

								$fetch_usp = $this->func_query->query_row(
									'lms_usp',
									'lms_usp_gp',
									'lms_usp.ug_id = lms_usp_gp.ug_id','',
									'lms_usp.emp_id = "'.$fetch_manage['emp_id'].'"');
								if(isset($fetch_usp) && $fetch_usp['ug_name_en']=="Learner" && $fetch_usp['ug_for']=="com_central"){
									$fetch_chkug = $this->func_query->query_row(
										'lms_usp_gp','','','',
										'ug_name_en="Learner (Manager)" and ug_for="com_central" and ug_isDelete="0" and ug_status="1"',
										'ug_id DESC');
									if(isset($fetch_chkug)){
										$arr_update_ug = array(
								...
								if(isset($fetch_usp) && $fetch_usp['ug_name_en']=="Learner" && $fetch_usp['ug_for']=="com_associated"){
									$fetch_chkug = $this->func_query->query_row(
										'lms_usp_gp','','','',
										'ug_name_en="Learner (Manager)" and ug_for="com_associated" and ug_isDelete="0" and ug_status="1"','ug_id DESC');
					...
					if($emp_manage_b){
							...
							if(isset($fetch_manage)&&$fetch_manage['is_manager']=="0"){
								...
								if(isset($fetch_usp)&&$fetch_usp['ug_name_en']=="Learner"&&$fetch_usp['ug_for']=="com_central"){
									$fetch_chkug = $this->func_query->query_row(
										'lms_usp_gp','','','',
										'ug_name_en="Learner (Manager)" and ug_for="com_central" and ug_isDelete="0" and ug_status="1"','ug_id DESC');
									if(isset($fetch_chkug)){
										$arr_update_ug = array(
											'ug_id' => $fetch_chkug['ug_id']
										);
			          ...
			          if( isset( $_FILES['img_profile']) ){
			              ...
			              if($pathBG!=""){
			                  ...
			                  if($_REQUEST['operation']=="Edit"){
			                      ...
			                      if(isset($fetch_img) && $fetch_img['img_profile']!=""){
			        ...
			        if(isset($_FILES['bgpic_user'])&&$_FILES['bgpic_user']!=""){
			          if( isset( $_FILES['bgpic_user']) ){
			              ...
			              if($pathBG!=""){
			                  ...
			                  if($_REQUEST['operation']=="Edit"){
			                      $fetch_img = $this->func_query->query_row('lms_usp','','','','u_id="'.$_REQUEST['u_id'].'"');
			                      if(isset($fetch_img) && $fetch_img['bgpic_user']!=""){
	...
	public function update_manager($emp_c_a,$emp_c_b){
		...
		if($emp_c_a!=""){
			$fetch_chkemp = $this->func_query->query_row(
				'lms_emp','','','',
				'emp_c="'.$emp_c_a.'" and emp_isDelete="0" and (depart_date="0000-00-00" or depart_date IS NULL)');
			if(isset($fetch_chkemp) ){
				$fetch_usp = $this->func_query->query_row(
					'lms_usp',
					'lms_usp_gp',
					'lms_usp.ug_id = lms_usp_gp.ug_id','',
					'lms_usp.emp_id = "'.$fetch_chkemp['emp_id'].'"');
				if(isset($fetch_usp) &&$fetch_usp['ug_name_en']=="Learner"&&$fetch_usp['ug_for']=="com_central"){
					$fetch_chkug = $this->func_query->query_row(
						'lms_usp_gp','','','',
						'ug_name_en="Learner (Manager)" and ug_for="com_central" and ug_isDelete="0" and ug_status="1"','ug_id DESC');
		...
		if($emp_c_b!=""){
			$fetch_chkemp = $this->func_query->query_row(
				'lms_emp','','','',
				'emp_c="'.$emp_c_b.'" and emp_isDelete="0" and (depart_date="0000-00-00" or depart_date IS NULL)');
			if(isset($fetch_chkemp) ){
				$fetch_usp = $this->func_query->query_row(
					'lms_usp',
					'lms_usp_gp',
	...
	public function delete_cosgroup_data(){
		...
		if(countArray($_REQUEST)>0){
			...
			$cgtitle = "";
			if($lang=="thai"){ 
			  $cgtitle = $fetch['cgtitle_th']!="" ? $fetch['cgtitle_th'] : $fetch['cgtitle_eng'];
			  $cgtitle = $cgtitle!="" ? $cgtitle : $fetch['cgtitle_jp'];
			}else if($lang=="english"){ 
			  $cgtitle = $fetch['cgtitle_eng']!="" ? $fetch['cgtitle_eng'] : $fetch['cgtitle_th'];
			  $cgtitle = $cgtitle!="" ? $cgtitle : $fetch['cgtitle_jp'];
			}else{
			  $cgtitle = $fetch['cgtitle_jp']!="" ? $fetch['cgtitle_jp'] : $fetch['cgtitle_eng'];
	...
	public function approve_cosgroup_data(){
		...
		if(countArray($_REQUEST)>0){
			...
			$cgtitle = "";
			if($lang=="thai"){ 
			  $cgtitle = isset($fetch_cg['cgtitle_th']) && $fetch_cg['cgtitle_th']!="" ? $fetch_cg['cgtitle_th'] : $fetch_cg['cgtitle_eng'];
			  $cgtitle = $cgtitle!="" ? $cgtitle : $fetch_cg['cgtitle_jp'];
			}else if($lang=="english"){ 
			  $cgtitle = isset($fetch_cg['cgtitle_eng']) && $fetch_cg['cgtitle_eng']!="" ? $fetch_cg['cgtitle_eng'] : $fetch_cg['cgtitle_th'];
			  $cgtitle = $cgtitle!="" ? $cgtitle : $fetch_cg['cgtitle_jp'];
			}else{
			  $cgtitle = isset($fetch_cg['cgtitle_jp']) && $fetch_cg['cgtitle_jp']!="" ? $fetch_cg['cgtitle_jp'] : $fetch_cg['cgtitle_eng'];
	...
	public function approve_cos_data(){
		...
		if(countArray($_REQUEST)>0){
    		...
    		$result = $this->func_query->query_row(
				'lms_cos',
				'lms_cos_detail',
				'lms_cos.cos_id = lms_cos_detail.cos_id','',
				'lms_cos.cos_id="'.$_REQUEST['cos_id'].'"');
    		if(isset($result)){
    		...
    		if($chkis_sendmail==1){
				...
				if(countArray($fetch_tc)>0){
					foreach ($fetch_tc as $key_tc => $value_tc) {
						...
						if(isset($fetch_formatmail)&&!in_array($fetch_user['email'], $arr_email)&&countArray($fetch_user)>0){
	...
	public function update_status_learner(){
		...
		$sess = $this->session->userdata("user");
          ...
          $fetch_enroll = $this->func_query->query_result('lms_cos_enroll','lms_cos','lms_cos.cos_id = lms_cos_enroll.cos_id','',
	...
	public function update_user_data(){
		...
		if(countArray($_REQUEST)>0){
		    ...
		    if($result['employ_date'] != "0000-00-00" && $result['employ_date'] != "" && $result['employ_date'] != null){
		    ...
		    if($result['inactivedate'] != "0000-00-00" && $result['inactivedate'] != "" && $result['inactivedate'] != null){
