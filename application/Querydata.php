...
class Querydata extends CI_Controller {
      ...
      if(isset($fetch_userp)){
        if($type=="da_logo_elearning"){
          if(is_file(ROOT_DIR."images/elearning_logo.png")){
              audit_unlink(ROOT_DIR."images/elearning_logo.png");
          }
        }else if($type=="da_logo_top"){
          if(is_file(ROOT_DIR."images/logo.png")){
              audit_unlink(ROOT_DIR."images/logo.png");
          }
        }else if($type=="da_logo_footer"){
          if(is_file(ROOT_DIR."images/logo_white.png")){
              audit_unlink(ROOT_DIR."images/logo_white.png");
          }
        }else if($type=="da_footer_background"){
          if(is_file(ROOT_DIR."images/bg.png")){
              audit_unlink(ROOT_DIR."images/bg.png");
          }
        }else if($type=="da_manual_sa_th"){
          if(is_file(ROOT_DIR."uploads/user_manual/manual_sa_th.pdf")){
              audit_unlink(ROOT_DIR."uploads/user_manual/manual_sa_th.pdf");
              $arr_update = array(
                'da_manual_sa_th'=>''
              );
        ...
        }else if($type=="da_manual_sa_eng"){
          if(is_file(ROOT_DIR."uploads/user_manual/manual_sa_eng.pdf")){
              audit_unlink(ROOT_DIR."uploads/user_manual/manual_sa_eng.pdf");
              $arr_update = array(
                'da_manual_sa_eng'=>''
              );
              $this->db->where('da_id','1');
        ...
        }else if($type=="da_manual_gr_th"){
          if(is_file(ROOT_DIR."uploads/user_manual/manual_gr_th.pdf")){
              audit_unlink(ROOT_DIR."uploads/user_manual/manual_gr_th.pdf");
              $arr_update = array(
                'da_manual_gr_th'=>''
              );
              $this->db->where('da_id','1');
        ...
        }else if($type=="da_manual_gr_eng"){
          if(is_file(ROOT_DIR."uploads/user_manual/manual_gr_eng.pdf")){
              audit_unlink(ROOT_DIR."uploads/user_manual/manual_gr_eng.pdf");
              $arr_update = array(
                'da_manual_gr_eng'=>''
              );
              $this->db->where('da_id','1');
        ...
        }else if($type=="da_manual_is_th"){
          if(is_file(ROOT_DIR."uploads/user_manual/manual_is_th.pdf")){
              audit_unlink(ROOT_DIR."uploads/user_manual/manual_is_th.pdf");
              $arr_update = array(
                'da_manual_is_th'=>''
              );
              $this->db->where('da_id','1');
        ...
        }else if($type=="da_manual_is_eng"){
          if(is_file(ROOT_DIR."uploads/user_manual/manual_is_eng.pdf")){
              audit_unlink(ROOT_DIR."uploads/user_manual/manual_is_eng.pdf");
              $arr_update = array(
                'da_manual_is_eng'=>''
              );
              $this->db->where('da_id','1');
        ...
        }else if($type=="da_manual_is_center_th"){
          if(is_file(ROOT_DIR."uploads/user_manual/manual_is_center_th.pdf")){
              audit_unlink(ROOT_DIR."uploads/user_manual/manual_is_center_th.pdf");
              $arr_update = array(
        ...
        }else if($type=="da_manual_is_center_eng"){
          if(is_file(ROOT_DIR."uploads/user_manual/manual_is_center_eng.pdf")){
              audit_unlink(ROOT_DIR."uploads/user_manual/manual_is_center_eng.pdf");
              $arr_update = array(
        ...
        }else if($type=="da_manual_is_affiliate_th"){
          if(is_file(ROOT_DIR."uploads/user_manual/manual_is_affiliate_th.pdf")){
              audit_unlink(ROOT_DIR."uploads/user_manual/manual_is_affiliate_th.pdf");
              $arr_update = array(
        ...
        }else if($type=="da_manual_is_affiliate_eng"){
          if(is_file(ROOT_DIR."uploads/user_manual/manual_is_affiliate_eng.pdf")){
              audit_unlink(ROOT_DIR."uploads/user_manual/manual_is_affiliate_eng.pdf");
              $arr_update = array(
        ...
        }else if($type=="da_manual_ln_th"){
          if(is_file(ROOT_DIR."uploads/user_manual/manual_ln_th.pdf")){
              audit_unlink(ROOT_DIR."uploads/user_manual/manual_ln_th.pdf");
              $arr_update = array(
                'da_manual_ln_th'=>''
              );
              $this->db->where('da_id','1');
        ...
        }else if($type=="da_manual_ln_eng"){
          if(is_file(ROOT_DIR."uploads/user_manual/manual_ln_eng.pdf")){
              audit_unlink(ROOT_DIR."uploads/user_manual/manual_ln_eng.pdf");
              $arr_update = array(
                'da_manual_ln_eng'=>''
              );
              $this->db->where('da_id','1');
    ...
    if(isset($fetch_chk)){
      ...
      if($condition!=""&&countArray($condition)>0){
        ...
        for ($i=0; $i < countArray($condition); $i++) { 
          $fetch_chkenroll = $this->func_query->query_row(
						'lms_cos_enroll','','','',
						'cos_id="'.$condition[$i].'" and emp_id = "'.$sess['emp_id'].'" and cosen_isDelete="0" and cosen_status="1" and cosen_finishtime!="0000-00-00 00:00:00"');
          if(isset($fetch_chkenroll)){
      ...
      if($pass==1){
            ...
            if($fetch_chkseat<=intval($fetch_chk['seat_count'])){
              $fetch_chkenroll = $this->func_query->query_row(
								'lms_cos_enroll','','','',
								'cos_id="'.$cos_id.'" and emp_id = "'.$sess['emp_id'].'" and cosen_isDelete="0" and cosen_status!="2"');
              if(!isset($fetch_chkenroll['cosen_status'])){
                $arr_insert = array(
                  'cos_id'							=>	$cos_id,
                  'emp_id'							=>	$sess['emp_id'],
                ...
                if(isset($fetch_cos_detail)){
                  if($fetch_cos_detail['date_start']!="0000-00-00 00:00:00"&&$fetch_cos_detail['date_end']!="0000-00-00 00:00:00"){
                    if($lang=="thai"){
											$periodstart = $fetch_cos_detail['date_start']!="0000-00-00 00:00:00" ? 
																			date('d ',strtotime($fetch_cos_detail['date_start'])).$thaimonth[intval(date('m',strtotime($fetch_cos_detail['date_start'])))]." ".
																			(date('Y',strtotime($fetch_cos_detail['date_start']))+543)." ".date('H:i',strtotime($fetch_cos_detail['date_start'])):"";
											$periodend = $fetch_cos_detail['date_end']!="0000-00-00 00:00:00" ? 
																			date('d ',strtotime($fetch_cos_detail['date_end'])).$thaimonth[intval(date('m',strtotime($fetch_cos_detail['date_end'])))]." ".
																			(date('Y',strtotime($fetch_cos_detail['date_end']))+543)." ".date('H:i',strtotime($fetch_cos_detail['date_end'])):"";
                    }else{
  ...
  public function detail_publicsurvey_report(){
      ...
      <table id="myTable_detail" width="" class="table table-bordered table-striped">
        <thead>
          <tr>
            ...
            <?php if(countArray($fetch_svde)>0){ $numloop = 1;
        ...
        <tbody>
          <?php
            ...
            if(countArray($fetch_detail)>0){ $numrows = 1;
                $arrEmpID = array();
                $arrEmpData = array();
                $arrCompany = array();
          ...
          <tr>
            ...
            <td><?php if($fetch_sv['sv_type']=="1"){ echo $arrEmpData[$value_detail['emp_id']]['fullname']; }else{ echo "User ".$numrows;} ?></td>
    ...
    <script type="text/javascript">
            ...
            $('#myTable_detail').DataTable({
              ...
              columnDefs: [{
                targets: [
                <?php if(countArray($fetch_svde)>0){ $numloop = 5;
  ...
  public function query_list_emp_reportsurveycos(){
    ...
    <div class="table-responsive">
      <table id="myTable_detail" width="100%" cellspacing="0" class="table table-bordered table-striped">
        <thead>
          <!-- <tr>
                <th colspan="<?php echo (countArray($fetch_svde))+2; ?>"><center><?php echo $lang=="thai"?"แบบสอบถามสำหรับรายวิชา ":"Survey for courses "; ?><?php echo $cname; ?></center></th>
          </tr>
          <tr>
  ...
  public function updateSaveSVTC(){
    ...
    if(isset($_REQUEST['svde_id'])&&countArray($_REQUEST['svde_id'])>0){
        for ($i=0; $i < countArray($_REQUEST['svde_id']); $i++) { 
            ...
            if(!isset($fetch_chk['svtc_firsttime'])){
              $arr_datamain = array(
                'sv_id' => $sv_id,
                'emp_id' => $sess['emp_id'],
      ...
      if(countArray($courses_total)>0){
        foreach ($courses_total as $key_list => $value_list) {
          ...
          if(!isset($fetch_status['cosen_status'])){
            $fetch_chk_ug = $this->func_query->query_row(
							'lms_cos_detail',
							'lms_cos_detail_ug',
            ...
            if(!isset($fetch_chk_ug['date_start'])){
              	unset($courses_total[$key_list]);
            }else{
								if($fetch_chk_ug['date_start']!="0000-00-00 00:00:00"&&$fetch_chk_ug['date_end']!="0000-00-00 00:00:00"){
									if(date('Y-m-d H:i',strtotime($fetch_chk_ug['date_start']))>date('Y-m-d H:i')){
           ...
           // $fetch_chk_ug = $this->func_query->query_row('lms_cos_detail','lms_cos_detail_ug','lms_cos_detail.cosde_id = lms_cos_detail_ug.cosde_id','','lms_cos_detail_ug.posi_id = "'.$sess['posi_id'].'" and lms_cos_detail.cos_id = "'.$value_list['cos_id'].'"');
            ...
            if($fetch_status['cosen_status']=="1" && $fetch_status['cosen_status_sub']=="1"){              
              /*if(($fetch_chk_ug['date_start']=="0000-00-00 00:00:00"&&$fetch_chk_ug['date_end']=="0000-00-00 00:00:00")||(date('Y-m-d H:i',strtotime($fetch_chk_ug['date_start']))<=date('Y-m-d H:i')&&date('Y-m-d H:i',strtotime($fetch_chk_ug['date_end']))>=date('Y-m-d H:i'))){
                ...
                if(isset($fetch_chk_ug['date_start'])){
                  if(($fetch_chk_ug['date_start']=="0000-00-00 00:00:00"&&$fetch_chk_ug['date_end']=="0000-00-00 00:00:00")||
										 (date('Y-m-d H:i',strtotime($fetch_chk_ug['date_end']))>=date('Y-m-d H:i')&&date('Y-m-d H:i',strtotime($fetch_chk_ug['date_start']))<=date('Y-m-d H:i'))){
  ...
  public function rechk_course_period(){
    ...
    if(isset($result)&&$result['date_start']!="0000-00-00 00:00:00"&&date('Y-m-d',strtotime($result['date_start']))<date('Y-m-d')){
        $result['isApprove'] = "0";
    }else{
        $result['isApprove'] = "1";
    }
  ...
  public function rechk_survey_period(){
    ...
    $result = $this->func_query->query_row('lms_sv','','','','sv_id="'.$sv_id.'"');
    $result['isApprove'] = "1";
    if(isset($result)&&$result['sv_open']!="0000-00-00 00:00:00"&&date('Y-m-d',strtotime($result['sv_open']))<date('Y-m-d')){
        $result['isApprove'] = "0";
    }
  ...
  public function save_question($qiz_id){
    ...
    if($type_qiz!=""){
      ...
      if(countArray($ques_id) > 0 && $cosen_id != "" && isset($sess["emp_id"])){
          $qiztc_id = "";
          $fetchEnroll = $this->func_query->query_row(
            'lms_cos_enroll','','','',
            'cosen_id="'.$cosen_id.'" and cosen_status = 1 and cosen_status_sub != 1 and cosen_lang != "" and cosen_isDelete = 0',
            'cosen_id DESC'
          );
          if (isset($fetchEnroll["cosen_id"])) {
              $fetch_chk = $this->func_query->query_row(
                'lms_qiz_tc','','','',
                'qiz_id="'.$qiz_id.'" and cosen_id="'.$cosen_id.'" and qiztc_isDelete="0" and time_finish="0000-00-00 00:00:00"',
                'qiztc_id DESC'
              );
              if(isset($fetch_chk)){
                  ...
                  $fetch_loop = $this->func_query->query_row(
                    'lms_qiz_tc','','','',
                    'qiz_id="'.$qiz_id.'" and cosen_id="'.$cosen_id.'" and qiztc_isDelete="0"',
                    'limit_val DESC'
                  );
                  $limit_val = isset($fetch_loop)?intval($fetch_loop['limit_val'])+1:1;
                  $arr_main = array(
                    'qiz_id'      => $qiz_id,
                    'emp_id'      => $fetchEnroll['emp_id'],
          ...
          if($qiztc_id!=""){
              for ($ques=0; $ques < countArray($ques_id); $ques++) { 
                  ...
                  $fetch_chk_ques = $this->func_query->query_row(
                    'lms_ques_tc','','','',
                    'qiztc_id="'.$qiztc_id.'" and cosen_id="'.$cosen_id.'" and qiz_id="'.$qiz_id.'" and ques_id="'.$ques_id[$ques].'"'
                  );
              ...
              if(isset($fetch_qiz)){
                ...
                if(countArray($fetch_qizall)>0){
                  ...
                  foreach ($fetch_qizall as $key_qizall => $value_qizall) {
                      $fetch_isSa = $this->func_query->numrows(
                        'lms_ques','','','',
                        'qiz_id="'.$value_qizall['qiz_id'].'" and ques_isDelete="0" and ques_type in ("sa","sub")'
                      );
                      if($fetch_isSa>0){
                        $isSaSub++;
                      }
                      $fetch_chk_tc = $this->func_query->query_row(
                        'lms_qiz_tc','','','',
                        'qiz_id="'.$value_qizall['qiz_id'].'" and cosen_id="'.$cosen_id.'" and qiztc_isDelete="0"',
                        'qiztc_id DESC'
                      );
                      
                      $fetch_chk_ques = $this->func_query->query_row(
                        'lms_ques','','','',
                        'qiz_id="'.$value_qizall['qiz_id'].'" and ques_isDelete="0" and ques_id in (select lms_ques_tc.ques_id from lms_ques_tc where cosen_id="'.$cosen_id.'" and qiztc_id = "'.$fetch_chk_tc['qiztc_id'].'")','',
                      ...
                      $num_loop = $this->func_query->numrows(
                        'lms_qiz_tc','','','',
                        'qiz_id="'.$value_qizall['qiz_id'].'" and qiztc_isDelete="0" and qiz_status="3" and cosen_id="'.$cosen_id.'"'
                      );
                      if(isset($fetch_chk_tc)){
                          $score_percos = isset($fetch_chk_ques['total_score']) && floatval($fetch_chk_ques['total_score'])>0?floatval($fetch_chk_tc['sum_score'])/floatval($fetch_chk_ques['total_score'])*100:100;
                  ...
                  if($isSaSub==0){
                    ...
                    if(isset($fetch_chkcos)){
                        ...
                        if(isset($fetch_chkbad['badges_condition'])=="P"){
                          $score_pass = floatval($fetch_chkcos['mina']);
                        }else{
                          if(isset($fetch_chkbad['badges_condition'])=="A"){
                            $score_pass = floatval($fetch_chkcos['mina']);
                          }else if(isset($fetch_chkbad['badges_condition'])=="B"){
                            $score_pass = floatval($fetch_chkcos['minb']);
                          }else if(isset($fetch_chkbad['badges_condition'])=="C"){
                            $score_pass = floatval($fetch_chkcos['minc']);
                          }else if(isset($fetch_chkbad['badges_condition'])=="D"){
                            $score_pass = floatval($fetch_chkcos['mind']);
                          }else{
                            $score_pass = 0;
                          }
                        }
                        if($score_cos>=$score_pass&&countArray(isset($fetch_chkbad))>0){
                            $output['isCert'] = "1";
                        }
                ...
                $fetch_isSa = $this->func_query->numrows(
                  'lms_ques','','','',
                  'qiz_id="'.$qiz_id.'" and ques_isDelete="0" and ques_type in ("sa","sub")'
                );
                if($fetch_isSa>0){
                    $isSaSub++;
                }
                ...
                if($num_qiztc>=countArray($fetch_qizall)){
                  $output['is_last'] = '1';
                  if($isSaSub==0){
                    $this->endcos($fetch_qiz['cos_id'], $cosen_id);
                  }
                }else{
                  $output['is_last'] = '0';
                  $output['isCert'] = "0";
  ...
  public function endcos($cos_id, $cosen_id){
      $lang = $this->session->userdata("lang") == null ? "english" : $this->session->userdata("lang");
      $this->lang->load($lang,$lang);
      $this->load->model('Function_query_model', 'func_query', FALSE);
      $this->load->model('Course_model', 'course', FALSE);
              ...
              if(countArray($fetch_qiz_query)>0){
                foreach ($fetch_qiz_query as $key_qiz_query => $value_qiz_query) {
                    ...
                    $fetch_chksh_lg = $this->func_query->numrows(
                      'lms_ques','','','',
                      'lms_ques.qiz_id="'.$value_qiz_query['qiz_id'].'" and ques_status="1" and ques_isDelete="0" and ques_type in ("sub","sa") and ques_id in (select lms_ques_tc.ques_id from lms_ques_tc where cosen_id="'.$cosen_id.'" and qiztc_id = "'.$numcheck_qizpass['qiztc_id'].'")'
                    );
                    if($fetch_chksh_lg>0){
                      $total_couse++;
                      $isEndTest = 1;
...
$lang = "english";
  ...
  public function recheckapprovemulti(){
    ...
    $this->func_query->loadDB();
      ...
      if(isset($_REQUEST['cg_id'])&&$_REQUEST['cg_id']!=""){
          ...
          if(isset($fetch_chk)&&$fetch_chk['cg_approve_by']!=""){
  ...
  public function permission_course(){
    ...
    $fetch_cos = $this->func_query->query_row('lms_cos','','','','cos_id="'.$cos_id.'"');
    $where = "";
    if(isset($fetch_comuser['com_admin']) && $fetch_comuser['com_admin']=="com_associated"){
    ...
    if(countArray($result_com)>0){
      ...
      ?>
      <div class="card m-b-0"  <?php if($fetch_cos['cos_approve']=="1"&&$sess['u_id']!="1"){?>style="pointer-events:none;"<?php } ?>>
        <div class="row">
          <div class="col-auto">
                      ...
                      if($value_posi['dep_id']==$value_dep['dep_id']){
                        ?>
                              <div class="col-12 chkall_<?php echo $value_com['com_id'] ?> chkdepall_<?php echo $value_posi['dep_id'] ?>"  <?php if($num_chkposi>0){ ?> style="margin-top:0px;"<?php }else{ ?>style="margin-bottom:2px;display: none;"<?php } ?>>
                                  <input type="checkbox" id="chkposi_<?php echo $value_posi['posi_id'] ?>" name="posi_var[]" class="filled-in chk-col-red chkcompany<?php echo $value_com['com_id']; ?> chkposiall_<?php echo $value_posi['dep_id'] ?>" onclick="onchkposi('<?php echo $value_posi['posi_id'] ?>','<?php echo $value_com['com_id']; ?>','<?php echo $value_posi['dep_id'] ?>')" data-com="<?php echo $value_com['com_id']; ?>" <?php if(in_array($value_posi['posi_id'], $arr_position)){ echo "checked";} ?> value="<?php echo $value_posi['posi_id'] ?>" data-dep="<?php echo $value_posi['dep_id']; ?>"  />
  ...
  public function update_sdve_question_detail_data(){
    ...
    if(countArray($_REQUEST)>0){
      ...
      if(isset($result['svde_type']) && $result['svde_type']=="multi"||$result['svde_type']=="2choice"){
  ...
  public function user_approve(){
    ...
    if(countArray($_REQUEST)>0){
      ...
      if(isset($_REQUEST['sv_id'])&&$_REQUEST['sv_id']!=""){
          ...
          if(isset($fetch_chk) && $fetch_chk['sv_userapprove']!=""){
  ...
  public function survey_data(){
    ...
    if(countArray($_REQUEST)>0){
      ...
      if(isset($fetch_status)&&$_REQUEST['type']=="real"){
      ...
      ?>
                  <div class="table-responsive">
                    <table class="table">
                        <thead>
                            <tr>
                                <th style="min-width: 250px;" class="align-middle"><p class="text-left"><?php echo $questiontxt; ?></p></th>
                                <th style="min-width: 100px;"><p class="text-center"><?php echo $choice_5txt; ?><br>5</p></th>
                                <th style="min-width: 100px;"><p class="text-center"><?php echo $choice_4txt; ?><br>4</p></th>
                                <th style="min-width: 100px;"><p class="text-center"><?php echo $choice_3txt; ?><br>3</p></th>
                                <th style="min-width: 100px;"><p class="text-center"><?php echo $choice_2txt; ?><br>2</p></th>
                                <th style="min-width: 150px;"><p class="text-center"><?php echo $choice_1txt; ?><br>1</p></th>
                                <th style="max-width: 200px;" class="align-middle"><p class="text-left"><?php echo $Suggestiontxt; ?></p></th>
                            </tr>
                        </thead>
                        <tbody>
                          <?php
                            if(countArray($result_head)>0){
                              ...
                              foreach ($result_head as $key_head => $value_head) {
                                ...
                                <tr>
                                  <td colspan="7"><h4 class="m-auto break-word"><b><?php echo $svde_heading; ?></b></h4></td>
                                </tr>
                                <?php
                          ...
                          <tr>
                                ...
                                <td class="text-center">
                                  <input <?php if($qnu_status=='1'){ echo "disabled"; }?> name="qnude_var[<?php echo $num; ?>]" type="radio" value="5" id="radio_<?php echo $num; ?>1" required class="with-gap radio-col-red" <?php if(isset($fetch_detailtc['qnude_var'])&&$fetch_detailtc['qnude_var']=="5"){echo "checked";} ?>><label for="radio_<?php echo $num; ?>1"></label>
                                </td>
                                <td class="text-center">
                                  <input <?php if($qnu_status=='1'){ echo "disabled"; }?> name="qnude_var[<?php echo $num; ?>]" type="radio" value="4" id="radio_<?php echo $num; ?>2" required class="with-gap radio-col-red" <?php if(isset($fetch_detailtc['qnude_var'])&&$fetch_detailtc['qnude_var']=="4"){echo "checked";} ?>><label for="radio_<?php echo $num; ?>2"></label>
                                </td>
                                <td class="text-center">
                                  <input <?php if($qnu_status=='1'){ echo "disabled"; }?> name="qnude_var[<?php echo $num; ?>]" type="radio" value="3" id="radio_<?php echo $num; ?>3" required class="with-gap radio-col-red" <?php if(isset($fetch_detailtc['qnude_var'])&&$fetch_detailtc['qnude_var']=="3"){echo "checked";} ?>><label for="radio_<?php echo $num; ?>3"></label>
                                </td>
                                <td class="text-center">
                                  <input <?php if($qnu_status=='1'){ echo "disabled"; }?> name="qnude_var[<?php echo $num; ?>]" type="radio" value="2" id="radio_<?php echo $num; ?>4" required class="with-gap radio-col-red" <?php if(isset($fetch_detailtc['qnude_var'])&&$fetch_detailtc['qnude_var']=="2"){echo "checked";} ?>><label for="radio_<?php echo $num; ?>4"></label>
                                </td>
                                <td class="text-center">
                                  <input <?php if($qnu_status=='1'){ echo "disabled"; }?> name="qnude_var[<?php echo $num; ?>]" type="radio" value="1" id="radio_<?php echo $num; ?>5" required class="with-gap radio-col-red" <?php if(isset($fetch_detailtc['qnude_var'])&&$fetch_detailtc['qnude_var']=="1"){echo "checked";} ?>><label for="radio_<?php echo $num; ?>5"></label>
                                </td>
                                <td>
                                  <textarea class="form-control" <?php if($qnu_status=='1'){ echo "disabled"; }?> name="qnude_suggestion[<?php echo $num; ?>]" id="qnude_suggestion" rows="3" style="min-width: 200px;"><?php if(isset($fetch_detailtc['qnude_suggestion'])&&$fetch_detailtc['qnude_suggestion']!=""){echo $fetch_detailtc['qnude_suggestion'];} ?></textarea>
                                </td>
                          </tr>
                          <?php  $num++;
                                }
                    ...
                    <?php if($result['sv_suggestion_status']=="1"){ ?>
                      ...
                      <textarea id="qnu_suggestion" name="qnu_suggestion" <?php if($qnu_status=='1'){ echo "disabled"; }?> class="form-control" rows="5"><?php if(isset($fetch_status['qnu_suggestion'])&&$fetch_status['qnu_suggestion']!=""&&$_REQUEST['type']=="real"){echo $fetch_status['qnu_suggestion'];} ?></textarea>
                    <?php } ?>
                  </div>
      <?php
    }
...
{
   return isset($input) && is_array($input) ? countArray($input) : 0;
}