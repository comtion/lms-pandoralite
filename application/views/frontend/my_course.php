        ...
        <div class="page-wrapper">
            ...
            <div class="container-fluid">
                ...
                <div class="card card-body row">
                  ...
                  <div class="col-md-12">
                    <?php if(countArray($list_course)>0){ ?>
                    <div class="row">
                    <?php if(countArray($list_coursegroup)>0){ ?>
                      ...
                      <div class="col-md-12 col-lg-9 card card-body">
                        <?php   $numgroup = 1;
                          ...
                          <div class="card-group div_cgheader<?php echo $value_cg['cg_id']; ?>">
                            ...
                            ?>
                              <div class="col-md-4 card <?php if($value_list['isCondition']=="1"){ ?>onCondition<?php } ?> course-item" data-name="<?php echo strtolower($value_list['cname']); ?>" data-group="<?php echo $value_cg['cg_id']; ?>" msg="<?php echo str_replace('_coursename_', $value_list['msgCondition'], label('condition_msg')); ?>">
                                <div class="card-img-top img-responsive <?php if($value_list['isRegister']=="1"){ ?>pointer<?php } ?>" alt="" <?php if($value_list['isRegister']=="1"&&$value_list['isCondition']=="0"){ ?>onclick="location.href='<?php echo REAL_PATH;?>/coursemain/detail/<?php echo $value_list['cos_id']; ?>';"<?php } ?> style="background-image:<?php if($value_list['cos_pic']!=""&&is_file(ROOT_DIR."uploads/course/".$value_list['cos_pic'])){ ?>url(<?php echo REAL_PATH;?>/uploads/course/<?php echo $value_list['cos_pic']; ?>) <?php }else{ ?>url(<?php echo REAL_PATH;?>/images/cover_course.jpg)<?php } ?>; overflow: hidden; background-size: contain; background-position: center; background-repeat: no-repeat; width: 100%; height: 215px;"></div>
                                ...
                                <div class="card-body">
                                  ...
                                  <p class="card-text mt-3"><?php echo label('lrn_b_course_period').': '; ?>
                                    <br>
                              <?php echo isset($value_list['txt_period_course']) ? $value_list['txt_period_course'] : lms_format_period_range($value_list['date_start'], $value_list['date_end'], $lang); ?>
                                  </p>
                            ...
                            <?php if($numgroup<countArray($list_coursegroup)){ ?>
                            <hr class="mb-4">
                            <?php } ?>
                          </div>
                        <?php   $numgroup++;
    ...
    <script type="text/javascript">
      function filterCoursesByName() {
        ...
        courseItems.forEach(item => {
          ...
          if (!groupVisibleMap[groupId]) groupVisibleMap[groupId] = 0;
          if (isVisible) groupVisibleMap[groupId]++;
        });

        // แสดงหรือซ่อนทั้งกลุ่มตามจำนวนที่เหลือ
        Object.keys(groupVisibleMap).forEach(groupId => {
          const visibleCount = groupVisibleMap[groupId];

          const headers = document.querySelectorAll(`.div_cgheader${groupId}`);
          headers.forEach(el => {
            el.style.display = visibleCount > 0 ? "block" : "none";
          });
        });

        // กลุ่มที่ไม่มี course-item แสดงเลย (เผื่อ fallback)
        const allGroupHeaders = document.querySelectorAll("[class^='div_cgheader']");

        allGroupHeaders.forEach(el => {
          const match = el.className.match(/div_cgheader(\d+)/);
          if (match) {
